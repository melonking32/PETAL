...
from lavis.models.blip2_models.model_loader_modify import ModelWrapper

@registry.register_model("blip2_t5_instruct_aurora_mixture_information")
class Blip2T5InstructAurora_Mixture_Information(Blip2Aurora):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.CP_U_1 = nn.Linear(768, self.R, bias=False)
        self.CP_V_1 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_1 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        nn.init.zeros_(self.CP_V_1.weight)
        nn.init.xavier_uniform_(self.CP_C_1)

        self.CP_U_Image_1 = nn.Linear(1408, self.R, bias=False)
        self.CP_V_Image_1 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_Image_1 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        # self.CP_C_Image=self.CP_C
        nn.init.zeros_(self.CP_V_Image_1.weight)
        nn.init.xavier_uniform_(self.CP_C_Image_1)
        
        self.CP_U_2 = nn.Linear(768, self.R, bias=False)
        self.CP_V_2 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_2 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        nn.init.zeros_(self.CP_V_2.weight)
        nn.init.xavier_uniform_(self.CP_C_2)

        self.CP_U_Image_2 = nn.Linear(1408, self.R, bias=False)
        self.CP_V_Image_2 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_Image_2 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        # self.CP_C_Image=self.CP_C
        nn.init.zeros_(self.CP_V_Image_2.weight)
        nn.init.xavier_uniform_(self.CP_C_Image_2)
        
        self.CP_U_3 = nn.Linear(768, self.R, bias=False)
        self.CP_V_3 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_3 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        nn.init.zeros_(self.CP_V_3.weight)
        nn.init.xavier_uniform_(self.CP_C_3)

        self.CP_U_Image_3 = nn.Linear(1408, self.R, bias=False)
        self.CP_V_Image_3 = nn.Linear(self.R, 768, bias=False)
        self.CP_C_Image_3 = nn.Parameter(torch.zeros([self.R, self.R, self.R], dtype=torch.float), requires_grad=True)
        # self.CP_C_Image=self.CP_C
        nn.init.zeros_(self.CP_V_Image_3.weight)
        nn.init.xavier_uniform_(self.CP_C_Image_3)
        
        self.Wrapper=ModelWrapper(num_cluster=len(self.t5_output_tokenizer))
        self.para_info = torch.nn.Parameter(torch.tensor(0.0).float(), requires_grad=True)
        # print(len(self.t5_output_tokenizer))
        for name, parameter in self.Wrapper.named_parameters():
            if 'weight' in name:
                nn.init.xavier_normal_(parameter)
            elif 'bias' in name:
                nn.init.constant_(parameter, 0)

        self.soft_instruct1=torch.randint(0,10000, (1, 16))
        self.soft_instruct2=torch.randint(10000,20000, (1, 24))
        self.soft_instruct3=torch.randint(20000,30000, (1, 32))
        
    def get_query_embedding_1(self,Instruct,image):
        ...
        if self.qformer_text_input:
            ...
            #     CP_V=self.CP_V_1, 
            #     CP_C=self.CP_C_1,
            #     CP_U_Image=self.CP_U_Image_1, 
            #     CP_V_Image=self.CP_V_Image_1, 
            #     CP_C_Image=self.CP_C_Image_1,
            # )
            Qformer_atts = torch.cat([query_atts,q_attn_mask],dim=1)
            query_output = self.Qformer.bert(
                input_ids,
        ...
        qformrt_output=query_output.last_hidden_state[:,:query_tokens.size(1),:]
        inputs_t5 = self.t5_proj(query_output.last_hidden_state[:,:query_tokens.size(1),:])
        
        atts_t5 = torch.ones(inputs_t5.size()[:-1], dtype=torch.long).to(image.device)
        return inputs_t5,atts_t5,image_embeds,qformrt_output
        
    def get_query_embedding_2(self,Instruct,image):
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)

        input_ids=self.soft_instruct2.expand(image_embeds.shape[0],-1).to(image.device)
        q_attn_mask=torch.ones(input_ids.size(), dtype=torch.long).to(image.device)
        ...
        if self.qformer_text_input:
            ...
            #     CP_V=self.CP_V_2, 
            #     CP_C=self.CP_C_2,
            #     CP_U_Image=self.CP_U_Image_2, 
            #     CP_V_Image=self.CP_V_Image_2, 
            #     CP_C_Image=self.CP_C_Image_2,
            # )
            query_output = self.Qformer.bert(
                input_ids,
                attention_mask=Qformer_atts,
    ...
    def get_query_embedding_3(self,Instruct,image):
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        
        input_ids=self.soft_instruct3.expand(image_embeds.shape[0],-1).to(image.device)
        q_attn_mask=torch.ones(input_ids.size(), dtype=torch.long).to(image.device)
        ...
        if self.qformer_text_input:
            ...
            #     CP_V=self.CP_V_3, 
            #     CP_C=self.CP_C_3,
            #     CP_U_Image=self.CP_U_Image_3, 
            #     CP_V_Image=self.CP_V_Image_3, 
            #     CP_C_Image=self.CP_C_Image_3,
            # )
            query_output = self.Qformer.bert(
                input_ids,
                attention_mask=Qformer_atts,
    ...
    def get_query_embedding_pre(self,Instruct,image):
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
    ...
    def forward(self, samples):
        ...
        image = samples["image"]
        inputs_t5_1,atts_t5,image_embeds,qformrt_output=self.get_query_embedding_1(samples['Instruct_1'],image)
        inputs_t5_2,_,_,_=self.get_query_embedding_2(samples['Instruct_2'],image)
        inputs_t5_3,_,_,_ =self.get_query_embedding_3(samples['Instruct_3'],image)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            wrapper_loss=self.Wrapper(image_embeds,qformrt_output,targets)
            ...
            return {"loss": loss+self.para_info*wrapper_loss}
    ...
    @torch.no_grad()
    def get_instruct_embedding_generate_1(self,query_tokens,Instruct,image):
        if self.qformer_text_input:
    ...
    def get_instruct_embedding_generate_2(self,query_tokens,Instruct,image):
        if self.qformer_text_input:
            # remove ocr tokens in q_former (for eval textvqa)
            # qformer_prompt = prompt
            # qformer_prompt = ['Question: ' + qp.split(' Question: ')[1] for qp in qformer_prompt]
    ...
    def get_instruct_embedding_generate_3(self,query_tokens,Instruct,image):
        if self.qformer_text_input:
            # remove ocr tokens in q_former (for eval textvqa)
            # qformer_prompt = prompt
            # qformer_prompt = ['Question: ' + qp.split(' Question: ')[1] for qp in qformer_prompt]
    ...
    def generate(
        ...
        inputs_t5_1, atts_t5=self.get_instruct_embedding_generate_1(query_tokens,samples['Instruct_1'],image)
        inputs_t5_2,_ =self.get_instruct_embedding_generate_2(query_tokens,samples['Instruct_2'],image)
        inputs_t5_3,_ =self.get_instruct_embedding_generate_3(query_tokens,samples['Instruct_3'],image)
